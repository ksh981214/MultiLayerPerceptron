...
class TwoLayerNet(object):
  ...
  def __init__(self, input_size, hidden_size, output_size, std=1e-4):
    ...
    """
    self.params = {}
    self.params['W1'] = std * torch.randn(input_size, hidden_size) # D x H
    self.params['b1'] = torch.zeros(hidden_size) # H,
    self.params['W2'] = std * torch.randn(hidden_size, output_size) # H x C
    self.params['b2'] = torch.zeros(output_size) # C,
  ...
  def loss(self, X, y=None):
    ...
    #############################################################################
    input = X #(N x D)
    h = torch.matmul(input ,W1) + b1 #(N, D) * (D, H) = (N, H) , 덧셈의 경우 자동 broadcasting
    #relu 
    for i in range(N):
    ...
    scores = torch.matmul(h ,W2) + b2# (N, H) * (H, C) = (N, C)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    result_loss = 0.0
    for i in range(N):
        result_loss -= torch.log(softmax[i,y[i]])
    loss = result_loss / N
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    for i in range(N):
        #loss도 평균으로 나누었으니까 1을 N으로 나누어 주어서 정답 라벨에서 빼줘야함.
        softmax_grad[i, y[i]] -= 1.0
    softmax_grad = softmax_grad / N
        
    grads['b2'] = torch.sum(softmax_grad, dim =0) # (1,C)
    grads['W2'] = torch.matmul(torch.t(h), softmax_grad) # (H,C)
    
    '''-----relu----'''
    
    grads_relu = torch.matmul(softmax_grad, torch.t(self.params['W2'])) # (N, H) 
    for i in range(N):
        for j in range(len(grads_relu[i])):
            if grads_relu[i,j] < 0:
                grads_relu[i,j] = 0.0
    
    grads['W1'] = torch.matmul(torch.t(X), grads_relu) # (D,H)
    grads['b1'] = torch.sum(grads_relu, dim = 0)
